...
int threadpool_map(void *pool, int size, void(*routine)(int n, void *),
                   void *arg, int flags)
{
    int err = 0;
    threadpool_map_t map = {
        .routine = routine,
        .arg = arg,
        .thread_count = ((tpool_t *) pool)->num_threads,
        .size = size,
    };

    sem_init(&map.done_indicator, 0, 0);

    for (int i = 0; i < ((tpool_t *) pool)->num_threads; i++)
        map.personal_pointers[i] = i;

    for (int i = 0; i < ((tpool_t *) pool)->num_threads; i++) {
        int _err = tpool_add_work(pool, threadpool_map_thread,
...
int threadpool_reduce(void *pool, threadpool_reduce_t *reduce)
{
    ...
    int err = threadpool_map(pool, ((tpool_t *) pool)->num_threads, threadpool_reduce_thread, &info, 0);
    if (err) return err;

    for (int i = 1; i < ((tpool_t *) pool)->num_threads; i++) {
